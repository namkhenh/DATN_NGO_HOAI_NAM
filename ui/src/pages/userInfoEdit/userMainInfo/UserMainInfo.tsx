...
import { IDistrict, IProvince, IUserAddress, IUserInfoViewModel, UserAddressModelProperty, UserInfoModelProperty, UserSexView } from '../../../model/apimodel/userInfo';
...
import { isStringEmpty, validateMaxLength, validateNumberField, validateRequire, validateRequireLimitCharacter } from '../../../utils/commonFunction';
...
function UserMainInfo(props: IUserMainInfoProps) {
    ...
    const  getProvinceOptions = async () => {
        const response: IProvince[] = await fetch('https://provinces.open-api.vn/api/?depth=1', {method: "GET",
        }).then((res) => res.json());
            ...
            {isLoading ?
                <div className="user-main-container" style={{ display: 'flex', flexWrap: 'wrap' }} >
                    {Array.from({ length: 5 }).map((item, index) => (
                        <>
                            <Skeleton key={index} variant="rounded" width={300} height={72} style={{ opacity: 1 - 0.15 * index }} />
                            <Skeleton key={index} variant="rounded" width={300} height={72} style={{ opacity: 1 - 0.15 * index }} />
                        </>
                    ))}
                </div > : 
                <div className='user-main-container'>
                    <div className="user-main-detail">
                        <TextField
                            ...
                            onChange={(_, value) => { onChangeOneField(UserInfoModelProperty.userName, value) }}
                    ...
                    <div className="user-main-detail">
                        <DatePicker
                            ...
                            onSelectDate={(date) => { onChangeOneField(UserInfoModelProperty.userDateBirth, `${date?.getMonth()}/${date?.getDay()}/${date?.getFullYear()}`) }}
                    ...
                    <div className="user-main-detail">
                        <TextField
                            ...
                            onChange={(_, value) => { onChangeOneField(UserInfoModelProperty.userIdentityNumber, value) }}
                    ...
                    <div className="user-main-detail">
                        <TextField
                            ...
                            onChange={(_, selected) => { onChangeAddress(UserAddressModelProperty.address, selected) }}
