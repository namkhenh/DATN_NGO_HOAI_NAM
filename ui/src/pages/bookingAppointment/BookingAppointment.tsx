...
import EventAvailableOutlinedIcon from '@mui/icons-material/EventAvailableOutlined'; import DeleteOutlineOutlinedIcon from '@mui/icons-material/DeleteOutlineOutlined';
import BookingPatientItem from '../../components/bookingPatientItem/BookingPatientItem';
import { AppointmentInfoModelProperty, IAppointmentInfo, IPatientProfileViewModel, PatientProfileDefaultView, PatientProfileModelProperty } from '../../model/apimodel/appointmentInfo';
...
import { isStringEmpty, validateRequire, validateRequireLimitCharacterForm } from '../../utils/commonFunction';
    ...
    return (
        <div id='booking-appointment' className='booking-appointment'>
            ...
            <div className="booking-appointment-container">
                <div className="booking-appointment">
                    ...
                    <div className="booking-appointment-sub-title">
                        Vui lòng chọn một trong các lịch khám có sẵn để xem chi tiết hoặc bấm vào <strong>Đặt khám mới</strong> để tạo lịch khám mới
                    </div>
                    ...
                    <div className="booking-new-booking-appointment">
                        {currentSelection !== 0 && <Button variant={ButtonVariantType.Outlined} startIcon={<DeleteOutlineOutlinedIcon />} sx={{ marginRight: '8px' }} color={ButtonColorType.Error} onClick={() => { handleActionPatientProfile("Delete", 0) }}>
                            Xóa người bệnh
                        </Button>}
                        <Button variant={ButtonVariantType.Outlined} startIcon={<AddIcon />} onClick={() => { handleActionPatientProfile("Add", 0)}}>
                            Thêm hồ sơ
                        </Button>
                    </div>
                </div>
                        ...
                        <div className="booking-appointment-wrap">
                            <div className="booking-appointment-item">
                                <DatePicker
                                    ...
                                    onSelectDate={(date) => { onChangeOneField(AppointmentInfoModelProperty.appointmentDate, `${date!.getMonth() + 1}/${date!.getDate()}/${date!.getFullYear()}`) }}
                            ...
                            <div className="booking-appointment-item">
                                <TextField
                                    ...
                                    onChange={(_, value) => { onChangeOneField(AppointmentInfoModelProperty.appointmentReason, value) }}
                                    errorMessage={errorMessageString.appointmentReason}
                                />
                            </div>
                            <div className="booking-button-wrap">
                                <Button variant={ButtonVariantType.Outlined} startIcon={<ClearOutlinedIcon />} color={ButtonColorType.Inherit} disabled={!isEdited} onClick={cancelBooking}>
                                    Hủy bỏ
                                </Button>
                                <Button variant={ButtonVariantType.Contained} startIcon={<EventAvailableOutlinedIcon />} onClick={openConfirmAppointment}>
                                    Đặt lịch
                                </Button>
                            </div>
                        </div>
            ...
            <DialogView
                title={'Xác nhận xóa'}
                hidden={!openDelete}
                customContent={<span>Bạn có chắc chắn muốn xóa hồ sơ bệnh nhân <strong>{currentSelection !== -1 ? allProfile[currentSelection].patientName : ''}</strong></span>}
